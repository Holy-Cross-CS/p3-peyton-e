...
# name and favorite color (i can't get it to automatically populate in the form with the correct cookies, which
# i think it because html files are static(?) but if you go from the form to the regular greeting page you'll see
# that it saves the name and color)

def handle_http_get_hello(req, conn):
    log("Handling http get hello request")

    #checking for cookies and setting default values if no cookies are present
    ...
    #collecting and parsing data from form
    ogpath = req.path
    if "?" in ogpath:
        ogpath, path = ogpath.split("?", 1)
        if "&" in path:
            namePair, colorPair = path.split("&", 1)
            names = namePair.split("=", 1)
            name = names[1].replace("+", " ")
            colors = colorPair.split("=", 1)
            favcolor = colors[1]
            
    if favcolor not in [ "coral", "crimson", "navy", "purple", "tomato", "teal", "wheat" ]:
        favcolor = None

    #dynamic hello page (colors change based on user's favorite color!)
    if favcolor == "coral":
    ...
    msg = '<html><head><title>Welcome!</title>'
    msg += '<style>body {background-color:'
    msg += f'{favcolor};'
    msg += f'color:{textcolor};'
    msg += '}</style></head><body>'
    msg += f'<h1> Welcome, {name} :)</h1>'
    msg += '<p>The current date is %s.\n</p>' % (time.strftime("%a, %d %b %Y %H:%M:%S %Z"))
    msg += f'<p>You have visited this page {hello_count} time(s) before.</p>'
    msg += f'<p>Maybe your favorite color is... {favcolor}??</p>'
    ...
    msg += f'  - <a href=http://{server_host}:{server_port}/hello>this page        </a><br>'
    msg += f'  - <a href=http://{server_host}:{server_port}/status>status and statistics       </a><br>'
    msg += f'  - <a href=http://{server_host}:{server_port}/quote>a random quote        </a><br>'
    msg += f'  - <a href=http://{server_host}:{server_port}/whoami>info about you!        </a><br>'
    msg += f'  - <a href=http://{server_host}:{server_port}/welcome.html>a welcome page </a><br>'
    msg += f'  - <a href=http://{server_host}:{server_port}/sharks>some pictures of sharks </a><br>'
    msg += f'  - <a href=http://{server_host}:{server_port}/index.html>some fancy web page   </a><br></p></body></html>'
    resp = Response("200 OK", "text/html", msg)

    #setting cookies
    if resp.cookies is None:
        resp.cookies = []
    resp.cookies.append(f"username={name}")
    resp.cookies.append(f"favcolor={favcolor}")
...
def handle_http_get_quote():
    ...
    msg += '<body><p>Here is a randomly generated quote from'
...
# handle_http_get_sharks() returns a response for GET /sharks,
# which generates images of sharks
# sometimes it takes a second to load, but I promise it's not crashing
def handle_http_get_sharks():
    log("Handling http get shark request")
    images = ["basking.jpeg","carpet.jpeg","greatwhite.jpeg","hammerhead.jpeg","lemon.jpeg",
              "nurse.jpeg","shortfinmako.jpeg","thresher.jpeg","tiger.jpg","whale.jpg","jack.jpeg"]
    captions = {"basking.jpeg":"Basking Shark","carpet.jpg":"Carpet Shark","greatwhite.jpeg":"Great White Shark",
                "hammerhead.jpeg":"Hammerhead Shark","lemon.jpeg":"Lemon Shark","nurse.jpeg":"Nurse Shark",
                "shortfinmako.jpeg":"Shortfin Mako Shark","thresher.jpeg":"Thresher Shark","tiger.jpeg":"Tiger Shark",
                "whale.jpg":"Whale Shark","jack.jpeg":"Jack Black (and friends) on a shark"}
    selected_image = random.choice(images)

    msg = '<html><head><title>Sharks!</title>'
    msg += '<style>body {background-color:rgb(249, 193, 225); color:rgb(25, 25, 112);}</style>'
    msg += '<style>body {font-family:courier;}'
    msg += 'img {height: 300px; width: auto; display: block; margin: 10px auto;}</style></head>'
    msg += '<h1 style="text-align:center;font-size:300%;">Here is a shark!</h1>'
    msg += f'<img src="{selected_image}" alt="Image of a shark">'
...
def handle_http_get(req, conn):
    ...
    if req.path == "/status":
        ...
        cookie_count = get_cookie(req.headers, "hello_count")
        if cookie_count is None:
            hello_count = 1
        else:
            hello_count = int(cookie_count) + 1
        resp = handle_http_get_hello(req, conn)
        if resp.cookies is None:
            resp.cookies = []
        resp.cookies.append(f"hello_count={hello_count}")
    elif req.path == "/whoami":
        user_agent = get_header_value(req.headers, "user-agent")
        cookies = get_header_value(req.headers, "Cookie")
        if cookies is None:
            cookies = "Not Specified"
        dnt = get_header_value(req.headers, "DNT")
        if dnt is None:
            dnt = "Not Specified"
        sec = get_header_value(req.headers, "sec-gpc")
        if sec is None:
            sec = "Not Specified"
        msg = '<html><head><title>Who Am I?</title>'
        msg += '<style>body {background-color: rgb(249, 193, 225); color: rgb(25, 25, 112);}</style></head>'
        msg += '<body><h1>Here is some info about you!: </h1><br>'
        msg += f'IP Address: {server_ip}<br><br>'
        msg += f'Server Port Number: {server_port}<br><br>'
        msg += f'Browser "User-Agent" String: {user_agent}<br><br>'
        msg += f'Cookies Present: {cookies}<br><br>'
        msg += f'Privacy Preferences, DNT: {dnt}, Sec-GPC: {sec}<br><br>'
        resp = Response("200 OK", "text/html", msg)
        return resp
    elif req.path == "/quote":
        ...
        if not os.path.isfile(file_path):
            msg = '<html><head><title>Page Options</title></head>'
            msg += '<body>Here are some pages you can visit: <br>'
            msg += f'  - <a href=http://{server_host}:{server_port}/hello>a nice greeting        </a><br>'
            msg += f'  - <a href=http://{server_host}:{server_port}/status>status and statistics       </a><br>'
            msg += f'  - <a href=http://{server_host}:{server_port}/whoami>info about you!        </a><br>'
            msg += f'  - <a href=http://{server_host}:{server_port}/quote>a random quote        </a><br>'
            msg += f'  - <a href=http://{server_host}:{server_port}/welcome.html>a welcome page </a><br>'
            msg += f'  - <a href=http://{server_host}:{server_port}/sharks>some pictures of sharks </a></body></html>'
            resp = Response("200 OK", "text/html", msg)
            return resp
        resp = handle_http_get_file(req_path)
    else:
